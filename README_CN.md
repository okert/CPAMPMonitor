...
- 菜单栏默认显示所有监控账号中的最低新鲜额度，也可以选择账号和额度窗口（例如 5 小时或周额度）；点击后查看账号详情。
- 支持 Codex、Antigravity、xAI，并保留 Claude OAuth 额度解析支持。
- 查看 5 小时、周额度、计费周期等窗口及重置时间。
- macOS 支持显示 Codex 剩余可用重置次数、逐次到期时间和剩余有效期；点击重置会先核实次数，再弹出消耗 1 次机会的确认框。确认后同步网关并刷新额度，失败不会自动重复消耗。
- 查看 CPAMP 提供的请求数、Token、成功率和计算成本历史。
- 支持菜单栏显示账号和额度窗口、账号筛选与排序、刷新间隔和告警阈值配置。
...
- 管理密钥权限由 CPAMP 服务决定；除额度和历史查询外，macOS 客户端可在用户确认后调用 Codex 重置和网关额度同步接口，不能替代服务端权限控制。
